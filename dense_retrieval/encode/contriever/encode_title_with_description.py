...
def encode_titles(corpus_file: str, output_file: str, batch_size: int = 32):
    ...
    def process_batch(batch):
        # Extract titles & descriptions from the batch
        titles_with_descriptions = [obj['title'] + ' ' + obj['description'] for obj in batch]

        # Encode using the Contriever model
        embeddings = encoder.encode(titles_with_descriptions)
    ...
    with jsonlines.open(corpus_file) as reader, jsonlines.open(output_file, mode='w') as writer:
        batch = []
        for obj in tqdm(reader, total=file_len(corpus_file), desc="Encoding titles with descriptions"):
            batch.append(obj)
