...
class BlipForQuestionAnswering(BlipForQuestionAnswering_hf):
    ...
    def forward(
        ...
        # reshape 
        query_result = query_logits[:, 0, :] ##(bsz, Vocab)
        product_result_0 = product_logits_0[:, 0, :] ##(bsz, Vocab)
        product_result_1 = product_logits_1[:, 0, :] ##(bsz, Vocab)
        product_result_0_kd = self.pooling(product_logits_0_kd, decoder_attention_mask) ##(bsz, Vocab)
        product_result_1_kd = self.pooling(product_logits_1_kd, decoder_attention_mask) ##(bsz, Vocab)
        ...
        MSELoss = nn.MSELoss()
        loss_0_kd = MSELoss(product_result_0, product_result_0_kd.detach())
        loss_1_kd = MSELoss(product_result_1, product_result_1_kd.detach())
