...
class BlipForProductEncoder(BlipPreTrainedModel):
    ...
    def encode(self, titles: List[str], descriptions: List[str], images_path: List[str] = None, **processor_kwargs):
        ...
        # get outputs
        outputs = self.generate(**inputs, return_dict_in_generate=True, output_scores=True)
        values = torch.cat([outputs.scores[i][:, None, :] for i in range(len(outputs.scores))], dim=1)
        decoded_tokens = outputs.sequences[:, 1:]
